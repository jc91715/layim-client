...
@section('scripts')
    ...
    <script>
        layui.use(['jquery','mobile'], function() {
            ...
            //
            //     //也可以直接跳转页面，如：
            //     location.href = '/mobile/msgbox'
            // });
            layim.on('detail', function(data1){
                console.log(data1); //获取当前会话对象，包含了很多所需要的信息
                //以查看群组信息（如成员）为例
                $.get("/group/api/getMembers?id="+data1.id,function (data) {
                    console.log(data)
                    layim.panel({
                        title: data.name + '群员信息' //标题
                        ,tpl: members.innerHTML //模版，基于laytpl语法
                        ,data: { //数据
                            members: data.data.list //假设rows为群组成员
                        }
                    });
                },'json')
                // ajax({
